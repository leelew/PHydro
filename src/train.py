            ...
            for epoch in pbar:
                ...
                for iter in range(0, cfg["niter"]):
                    x_batch, y_batch, aux_batch, \
                        mean_batch, std_batch = load_train_data(
                            cfg, x, y, aux, scaler)
                    with tf.GradientTape() as tape:
                        ...
                        phy_loss = MassConserveLoss(cfg, mean_batch, std_batch)(aux_batch, pred)
                        MCLoss += phy_loss
                ...
                if valid_split:
                    ...
                    if epoch % 20 == 0:
                        ...
                        loss_str = '\033[1;31m%s\033[0m' % \
                            "Epoch {} Val NSE {:.3f} MC Loss {:.3f} time {:.2f}".format(
                                epoch, val_acc, MC_valid_loss/x_valid.shape[0], t1-t0)
                        print(loss_str)
